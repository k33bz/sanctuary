...
| Feral eggs | a feral hen's eggs become **Feral Eggs** — tier in the name's color, bloodline depth as 0–5★. Most hatch plain chicks; stars raise the odds of a destined chick at the parent tier ±1, which turns at adulthood and lays one star deeper. Five lucky generations to 5★ Nightmare stock |
...
| `/sanctuary anchor time add\|set <hours> [selector]` | Adjust charge: nearest anchor, or by selector — full anchor UUID = exactly one, 8-char/owner name/`wild*cards` = all matches |
| `/sanctuary anchor list [selector]` | List anchors with their ids; filter by id, owner, or wildcard (`893e*`, `MadeUpNam*`) |
